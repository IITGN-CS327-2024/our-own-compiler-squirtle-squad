...
class semanticCheck(NodeVisitor):
  ...
  def visit_FunctionCall(self, node):
    ...
    #! remaining -> check the parameters and return type
  ...
  def visit_PrintStatement(self, node):
    # TODO we can have two ways -> we allow both int and string or we allow only one type
    # one thing we need to take care of is that in this complex val child -> format func
    child = self.visit(node.children[1])
    if child != tc.Number and child != tc.String:
  ...
  def visit_ConsOp(self, node):
    ...
    if record['type'] != 'array_variable' or record['type'] != 'tuple_variable':
      raise Exception(lexeme," is not declared as a tuple or array")
    child = self.visit(node.children[3])
  ...
  def visit_CompCondition(self, node):
    left = self.visit(node.children[0])
    right = self.visit(node.children[2])
    # TODO are we comaprint arrays and tuples
